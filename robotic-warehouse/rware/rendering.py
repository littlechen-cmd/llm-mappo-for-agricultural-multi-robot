...
_CHARGER_COLOR = (31, 119, 180)
_PICKER_COLOR = (148, 103, 189)
_PICK_DOCK_COLOR = (44, 160, 44)
_DEAD_AGENT_COLOR = (80, 80, 80)
_LOCKED_AGENT_COLOR = (214, 39, 40)
...
class Viewer(object):
    ...
    def render(self, env, return_rgb_array=False):
        ...
        self._draw_heterogeneous_stations(env)
    ...
    def _draw_shelfs(self, env):
        ...
        for shelf in env.shelfs:
            if not getattr(shelf, "active", True):
                continue
            x, y = shelf.x, shelf.y
    ...
    def _draw_heterogeneous_stations(self, env):
        chargers = getattr(env, "charging_stations", [])
        pickers = getattr(env, "picking_robots", [])
        docks = getattr(env, "picking_docks", [])
        if not chargers and not pickers and not docks:
            return

        def draw_cell(x, y, color, label):
            y = self.rows - y - 1
            left = (self.grid_size + 1) * x + 1
            bottom = (self.grid_size + 1) * y + 1
            pyglet.graphics.draw(
                4,
                gl.GL_QUADS,
                ("v2f", (left, bottom, left + self.grid_size, bottom,
                         left + self.grid_size, bottom + self.grid_size,
                         left, bottom + self.grid_size)),
                ("c3B", 4 * color),
            )
            pyglet.text.Label(
                label,
                ...
                font_size=13,
                bold=True,
                x=left + self.grid_size / 2,
                y=bottom + self.grid_size / 2,
                anchor_x="center",
                anchor_y="center",
                color=(*_WHITE, 255),
            ).draw()
        ...
        for x, y in chargers:
            draw_cell(x, y, _CHARGER_COLOR, "C")
        for picker in pickers:
            draw_cell(picker.x, picker.y, _PICKER_COLOR, "P")
        for x, y in docks:
            draw_cell(x, y, _PICK_DOCK_COLOR, "D")
    ...
    def _draw_agents(self, env):
        ...
        for agent in env.agents:
            ...
            if getattr(agent, "dead", False):
                draw_color = _DEAD_AGENT_COLOR
            elif getattr(agent, "locked", False):
                draw_color = _LOCKED_AGENT_COLOR
            else:
        ...
        for agent in env.agents:
            if not hasattr(agent, "battery"):
                continue
            col, row = agent.x, self.rows - agent.y - 1
            pyglet.text.Label(
